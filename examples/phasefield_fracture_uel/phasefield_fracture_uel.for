...
C     Material: PhaseFieldFractureMaterial
C     Fields: u, d
...
C     --- Local parameters ---
      ...
      DOUBLE PRECISION :: d_node(4)
      DOUBLE PRECISION :: d_old(4)
      INTEGER :: edof_d(4)
...
C     --- Gauss point field values ---
      ...
      DOUBLE PRECISION :: d_gp, d_old_gp
      DOUBLE PRECISION :: grad_d(3)
...
C     Complex temps for real material evaluation
      ...
      DOUBLE COMPLEX :: dz_r
      DOUBLE COMPLEX :: grad_d_r(3)

C     --- Tangent arrays ---
      DOUBLE PRECISION :: dPK1_dF(3,3,3,3)
      DOUBLE PRECISION :: dPK1_dd(3,3)
      DOUBLE PRECISION :: dphase_flux_dgrad_d(3,3)
      DOUBLE PRECISION :: dphase_storage_dd
...
C       Real material evaluation (for RHS)
        ...
        dz_r = DCMPLX(d_gp, 0.0d0)
        CALL real2complex3(grad_d, grad_d_r)
        CALL phasefieldfracturematerial_stress_PK1(Fz_r, dz_r, PROPS,
     &Pz_eval)
        DO i = 1, 3
          DO j = 1, 3
          ...
          END DO
        END DO
        CALL phasefieldfracturematerial_phase_flux(grad_d_r, PROPS,
     &phase_fluxz_eval)
        DO i = 1, 3
          phase_flux_real(i) = DBLE(phase_fluxz_eval(i))
        END DO
        CALL phasefieldfracturematerial_phase_storage(dz_r, PROPS,
     &phase_storagez_eval)
        phase_storage_real = DBLE(phase_storagez_eval)

C       CS tangent engine
        CALL phasefieldfracturematerial_cs_tangents(F, d_gp,
     &     grad_d, F_old, PROPS, dPK1_dF, dPK1_dd,
     &     dphase_flux_dgrad_d, dphase_storage_dd)
...
C       dphase_flux_dgrad_d (sign=-1, grad-grad)
        DO ii_v = 1, 4
          DO jj_v = 1, 4
            row = edof_d(ii_v)
            col = edof_d(jj_v)
...
C       dphase_storage_dd (sign=+1, value-value)
        DO ii_v = 1, 4
          DO jj_v = 1, 4
            row = edof_d(ii_v)
            col = edof_d(jj_v)
...
C======================================================================
      SUBROUTINE phasefieldfracturematerial_stress_PK1(F, d, props,
     &P_out)
      IMPLICIT NONE
      DOUBLE COMPLEX, INTENT(IN) :: F(3,3)
      DOUBLE COMPLEX, INTENT(IN) :: d
      DOUBLE PRECISION, INTENT(IN) :: props(6)
     ...
     &DCMPLX(2.0d0, 0.0d0)) + DCMPLX(props(5), 0.0d0))
      ...
      DO ii = 1, 3
        DO jj = 1, 3
          P_out(ii,jj) = (g_d * P0(ii,jj))
        END DO
      END DO

      RETURN
      END SUBROUTINE phasefieldfracturematerial_stress_PK1

      SUBROUTINE phasefieldfracturematerial_phase_flux(grad_d, props,
     &phase_flux_out)
      IMPLICIT NONE
      DOUBLE COMPLEX, INTENT(IN) :: grad_d(3)
      DOUBLE PRECISION, INTENT(IN) :: props(6)
      ...
      DO ii = 1, 3
        phase_flux_out(ii) = (((-DCMPLX(props(3), 0.0d0)) *
     &DCMPLX(props(4), 0.0d0)) * grad_d(ii))
      END DO

      RETURN
      END SUBROUTINE phasefieldfracturematerial_phase_flux

      SUBROUTINE phasefieldfracturematerial_phase_storage(d, props,
     &phase_storage_out)
      IMPLICIT NONE
      DOUBLE COMPLEX, INTENT(IN) :: d
      DOUBLE PRECISION, INTENT(IN) :: props(6)
      ...
      phase_storage_out = (((DCMPLX(props(3), 0.0d0) / DCMPLX(props(4),
     &0.0d0)) * d) - ((DCMPLX(2.000000000000000d+00, 0.0d0) *
     &(DCMPLX(1.000000000000000d+00, 0.0d0) - d)) * DCMPLX(props(6),
     &0.0d0)))

      RETURN
      END SUBROUTINE phasefieldfracturematerial_phase_storage


C======================================================================
C     Section 3: Complex-step tangent engine
C======================================================================
      SUBROUTINE phasefieldfracturematerial_cs_tangents(
     &   F, d, grad_d, F_old, props, dPK1_dF, dPK1_dd,
     &   dphase_flux_dgrad_d, dphase_storage_dd)
...
C     --- Inputs (all DOUBLE PRECISION) ---
      ...
      DOUBLE PRECISION, INTENT(IN) :: grad_d(3)
      DOUBLE PRECISION, INTENT(IN) :: F_old(3,3)
      DOUBLE PRECISION, INTENT(IN) :: props(6)

C     --- Outputs (all DOUBLE PRECISION) ---
      DOUBLE PRECISION, INTENT(OUT) :: dPK1_dF(3,3,3,3)
      DOUBLE PRECISION, INTENT(OUT) :: dPK1_dd(3,3)
      DOUBLE PRECISION, INTENT(OUT) :: dphase_flux_dgrad_d(3,3)
      DOUBLE PRECISION, INTENT(OUT) :: dphase_storage_dd
...
C     === dPK1_dF: d(stress_PK1)/d(F) ===
      DO k = 1, 3
        DO l = 1, 3
          ...
          dz = DCMPLX(d, 0.0d0)
          CALL real2complex3(grad_d, grad_d_z)
          Fz(k,l) = Fz(k,l) + DCMPLX(0.0d0, CS_H)
          CALL phasefieldfracturematerial_stress_PK1(Fz, dz, props, PKz)
          DO i = 1, 3
            DO j = 1, 3
...
C     === dPK1_dd: d(stress_PK1)/d(d) ===
      ...
      DO i = 1, 3
        DO j = 1, 3
          dPK1_dd(i,j) = AIMAG(PKz(i,j)) / CS_H
        END DO
      END DO

C     === dphase_flux_dgrad_d: d(phase_flux)/d(grad_d) ===
      DO k = 1, 3
        CALL real2complex33(F, Fz)
        dz = DCMPLX(d, 0.0d0)
        CALL real2complex3(grad_d, grad_d_z)
        grad_d_z(k) = grad_d_z(k) + DCMPLX(0.0d0, CS_H)
        CALL phasefieldfracturematerial_phase_flux(grad_d_z, props, jRz)
        DO i = 1, 3
          dphase_flux_dgrad_d(i,k) = AIMAG(jRz(i)) / CS_H
        END DO
      END DO

C     === dphase_storage_dd: d(phase_storage)/d(d) ===
      CALL real2complex33(F, Fz)
      dz = DCMPLX(d, CS_H)
      CALL real2complex3(grad_d, grad_d_z)
      CALL phasefieldfracturematerial_phase_storage(dz, props, cdotz)
      dphase_storage_dd = AIMAG(cdotz) / CS_H

      RETURN
      END SUBROUTINE phasefieldfracturematerial_cs_tangents
