...
C     Fields: global u; local p stored in SVARS(1)
...
C======================================================================
     ...
     &  JPROPS, NJPROP, PERIOD)
      ...
      INTEGER, PARAMETER :: ndim = 2, NNODE_E = 4, NGP = 4
...
C     Local Newton for element pressure
     ...
     &      sh4, dshxi4)
          CALL map_grad_2d(dshxi4, coords2, NNODE_E, dsh4,
     &      detJ, Jinv, stat)
          IF (stat .EQ. 0) THEN
            PNEWDT = 0.25d0
            RETURN
     ...
     &            + u_old(i,ii_v)*dsh4(ii_v,j)
            ...
            END DO
          END DO
          CALL localp_eval_tangents(F, p_elem,
     &      F_old, p_old, PROPS, dt_safe,
     &      P_real, rp_real,
     &      dP_dF, dP_dp, drp_dF, drp_dp)
...
C     Assemble full x-p blocks, x = [u] (mechanical) or [u, mu] (gel)
     ...
     &          + u_old(i,ii_v)*dsh4(ii_v,j)
          ...
          END DO
        END DO
        CALL localp_eval_tangents(F, p_elem, F_old, p_old,
     &    PROPS, dt_safe,
     &    P_real, rp_real,
     &    dP_dF, dP_dp, drp_dF, drp_dp)
        detF_diag = det33d(F)
...
C======================================================================
      SUBROUTINE neohookeanup_stress_PK1(F, p, props, P_out)
      IMPLICIT NONE
     ...
     &jj))) + (p * AT4(ii,jj)))
      ...
      RETURN
      END SUBROUTINE neohookeanup_stress_PK1

      SUBROUTINE neohookeanup_pressure_resid(F, p, props, rp_out)
      IMPLICIT NONE
      DOUBLE COMPLEX, INTENT(IN) :: F(3,3)
      DOUBLE COMPLEX, INTENT(IN) :: p
      DOUBLE PRECISION, INTENT(IN) :: props(2)
...
C======================================================================
     ...
     &  F, p, F_old, p_old, PROPS, dt,
     &  P_real, rp_real,
     &  dP_dF, dP_dp, drp_dF, drp_dp)
      IMPLICIT NONE
      DOUBLE PRECISION, INTENT(IN) :: F(3,3), p
      DOUBLE PRECISION, INTENT(IN) :: F_old(3,3), p_old
      DOUBLE PRECISION, INTENT(IN) :: PROPS(2), dt
