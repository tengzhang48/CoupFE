...
C     --- Local parameters ---
      ...
      DOUBLE PRECISION :: mu_node(4)
      DOUBLE PRECISION :: mu_old(4)
      INTEGER :: edof_mu(4)
...
C       Interpolate fields at Gauss point
     ...
     &          + u_old(i,ii_v)*dsh4(ii_v,j)
        ...
        DO ii_v = 1, 4
          mu_gp = mu_gp
     &      + mu_node(ii_v)*sh4(ii_v)
          mu_old_gp = mu_old_gp
     &      + mu_old(ii_v)*sh4(ii_v)
        END DO
        grad_mu(1) = 0.0d0
        grad_mu(2) = 0.0d0
        grad_mu(3) = 0.0d0
...
C       Real material evaluation (for RHS)
        ...
        CALL simplegelmaterial_stress_PK1(Fz_r, muz_r, PROPS, Pz_eval)
        DO i = 1, 3
          DO j = 1, 3
            P_real(i,j) = DBLE(Pz_eval(i,j))
          END DO
        END DO
        CALL simplegelmaterial_solvent_flux(Fz_r, muz_r, grad_mu_r,
     &PROPS, jRz_eval)
        DO i = 1, 3
          jR_real(i) = DBLE(jRz_eval(i))
        END DO
        CALL simplegelmaterial_solvent_storage(Fz_r, Fold_r, muz_r,
     &dt_safe, PROPS, cdotz_eval)
        cdot_real = DBLE(cdotz_eval)

C       CS tangent engine
        CALL simplegelmaterial_cs_tangents(F, mu_gp, grad_mu,
     &     F_old, PROPS, dt_safe, dPK1_dF, dPK1_dmu, dflux_dF,
     &     dflux_dgrad_mu, dflux_dmu, dstorage_dF, dstorage_dmu)
...
C       dstorage_dmu (sign=+1, value-value)
        DO ii_v = 1, 4
          DO jj_v = 1, 4
            row = edof_mu(ii_v)
            col = edof_mu(jj_v)
...
C======================================================================
      SUBROUTINE simplegelmaterial_stress_PK1(F, mu, props, P_out)
      IMPLICIT NONE
     ...
     &EYE1(ii,jj))) + (mu * EYE2(ii,jj)))
        END DO
      END DO

      RETURN
      END SUBROUTINE simplegelmaterial_stress_PK1

      SUBROUTINE simplegelmaterial_solvent_flux(F, mu, grad_mu, props,
     &jR_out)
      IMPLICIT NONE
      DOUBLE COMPLEX, INTENT(IN) :: F(3,3)
      DOUBLE COMPLEX, INTENT(IN) :: mu
      ...
      DO ii = 1, 3
        jR_out(ii) = ((-DCMPLX(props(2), 0.0d0)) * grad_mu(ii))
      END DO

      RETURN
      END SUBROUTINE simplegelmaterial_solvent_flux

      SUBROUTINE simplegelmaterial_solvent_storage(F, F_old, mu, dt,
     &props, cdot_out)
      IMPLICIT NONE
      DOUBLE COMPLEX, INTENT(IN) :: F(3,3)
      ...
      RETURN
      END SUBROUTINE simplegelmaterial_solvent_storage


C======================================================================
C     Section 3: Complex-step tangent engine
C======================================================================
      SUBROUTINE simplegelmaterial_cs_tangents(
     &   F, mu, grad_mu, F_old, props, dt, dPK1_dF, dPK1_dmu,
     &   dflux_dF, dflux_dgrad_mu, dflux_dmu, dstorage_dF,
     &   dstorage_dmu)
...
C     --- Outputs (all DOUBLE PRECISION) ---
      ...
      DOUBLE PRECISION, INTENT(OUT) :: dstorage_dmu
...
C     === dPK1_dF: d(stress_PK1)/d(F) ===
      DO k = 1, 3
        DO l = 1, 3
          ...
          CALL simplegelmaterial_stress_PK1(Fz, muz, props, PKz)
          DO i = 1, 3
            DO j = 1, 3
...
C     === dflux_dF: d(solvent_flux)/d(F) ===
      DO k = 1, 3
        DO l = 1, 3
          ...
          CALL real2complex3(grad_mu, grad_mu_z)
          Fz(k,l) = Fz(k,l) + DCMPLX(0.0d0, CS_H)
          CALL simplegelmaterial_solvent_flux(Fz, muz, grad_mu_z, props,
     & jRz)
          DO i = 1, 3
...
C     === dstorage_dF: d(solvent_storage)/d(F) ===
      DO k = 1, 3
        DO l = 1, 3
          ...
          CALL real2complex3(grad_mu, grad_mu_z)
          Fz(k,l) = Fz(k,l) + DCMPLX(0.0d0, CS_H)
          CALL simplegelmaterial_solvent_storage(Fz, F_old_z, muz, dt,
     &props, cdotz)
          dstorage_dF(k,l) = AIMAG(cdotz) / CS_H
        END DO
      END DO

C     === dstorage_dmu: d(solvent_storage)/d(mu) ===
      CALL real2complex33(F, Fz)
      muz = DCMPLX(mu, CS_H)
      CALL real2complex3(grad_mu, grad_mu_z)
      CALL simplegelmaterial_solvent_storage(Fz, F_old_z, muz, dt,
     &props, cdotz)
      dstorage_dmu = AIMAG(cdotz) / CS_H

      RETURN
      END SUBROUTINE simplegelmaterial_cs_tangents
