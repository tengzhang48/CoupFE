...
| `exact_stick_friction` | **READY — study** | Small-scale global Coulomb exact-stick and analytic onset checks. The slip cap scales the all-stick multiplier pattern to one normalized global resultant and is checked through an independently recovered reaction, negative friction work, and under-cap/reversed-direction broken controls. Method context: P. Alart and A. Curnier, *Computer Methods in Applied Mechanics and Engineering* 92(3), 353–375 (1991), DOI `10.1016/0045-7825(91)90022-X`. |
...
| `j2_plasticity_uel` | **READY — codegen proof** | Deliberately small-strain associative J2 example with elastic and active-plastic material-point checks, yield-surface closure, generated-kernel state/commit checks, and a broken-return control. It is not evidence for finite-strain plasticity or a named structural benchmark. |
...
| `neo_hookean_block` | **READY** | Compiled finite-strain block solve checked against the locally derived traction-free lateral stretch. The public physical `(G,K)` pair is separately gated against the retained raw `(G,lambda)` kernel convention; no external data. |
...
| `scalar_diffusion_uel` | **READY — codegen proof** | Synthetic coupled thermo-mechanical/Fourier generator with locally stated equations and implementation checks. Its `alpha` is log-volumetric and `T` is a change from an implicit zero reference, matching the sibling thermo-mechanical example; it is not a reproduction of an external model. |
...
| `small_strain_viscoelastic_umat` | **READY — codegen proof** | Standard-linear-solid UMAT generation with tensor-history, relaxation/path (including a non-default relaxation-time gate), broken-control, deterministic-regeneration, and compiler checks. Source port: MIT-licensed `abaqus_ufl` commit `0f525339db1aad70e9f8f4825a02c1164f0da7a0`. This is not a fitted material or structural benchmark. |
| `stabilized_tet4` | **RESEARCH — paper form** | Project-authored stabilized mixed `u-theta` Tet4 declaration with analytic derivative/homogeneous-state, explicit major-symmetry, normal-tolerance tangent, generation, and compiler checks. Source port: MIT-licensed `abaqus_ufl` commit `0f525339db1aad70e9f8f4825a02c1164f0da7a0`. Formulation reference: G. Scovazzi, R. Zorrilla, and R. Rossi, *Computer Methods in Applied Mechanics and Engineering* 412 (2023), 116076, DOI `10.1016/j.cma.2023.116076`. No current Abaqus block reproduction is claimed. |
| `thermo_mechanics_quad8` | **READY — codegen proof** | Synthetic coupled displacement-temperature Quad8 declaration with locally stated equations and scoped implementation checks. Its `alpha` is explicitly a log-volumetric coefficient and `T` is a change from an implicit zero reference; it is not a named external-model reproduction. |
