...
| **Anisotropic finite-strain** (Holzapfel–Ogden myocardium: fiber/sheet structural-tensor state, `I8fs` coupling, active stress, viscous) | ◐ implemented in `CoupFE-cardiac-elastodynamics` (separate repo): local material/operator gates exist, but the historical Case A backward-Euler comparison was not retained and does not validate the current Newmark default; Case B quantitative agreement and diagnosis remain open |
