...
Internal variables (state_vars):
  ...
  Chester's umat_simpleFeFp.for pattern), extends the material subroutine
...
def _state_var_info(material):
    """
    Compute state variable layout from material.state_schema.

    Returns OrderedDict:  name -> {'init': value, 'size': int, 'shape': str,
                                   'offset': int, 'init_prop_index': int|None}
      shape is 'scalar', 'vector', or 'tensor'
      offset is the flat index within the per-GP state block.

    Returns empty OrderedDict if material has no state_vars.
    """
    schema = getattr(material, 'state_schema', None)
    if schema is None:
        # Fallback for code that calls the function on a class or plain object
        if hasattr(material, '_normalize_state_vars'):
            schema = material._normalize_state_vars()
        else:
            return OrderedDict()
    if not schema:
        return OrderedDict()
...
class FortranTranslator(ast.NodeVisitor):
    ...
    def _visit_stmt(self, node):
        if isinstance(node, ast.Return):
            ...
            raise NotImplementedError(
                ...
                "See docs/lessons_learned.md section 8.1.")
    ...
    def _visit_expr(self, node):
        ...
        if isinstance(node, ast.Constant):
            ...
            if func_name == 'transpose':
                if len(args) != 1:
                    raise NotImplementedError(
                        "transpose requires exactly 1 argument")
                tmp = self._new_temp('TR')
                self._add_decl('DOUBLE COMPLEX', tmp, '(3,3)')
                self._var_kinds[tmp] = 'tensor'
                self._add_stmt(f'CALL transpose33z({args[0]}, {tmp})')
                return tmp, 'tensor'
            elif func_name == 'sum':
                if len(args) != 1:
                    raise NotImplementedError(
                        "sum requires exactly 1 argument")
                return f'SUM({args[0]})', 'scalar'
            elif func_name == 'where':
                if len(args) != 3:
                    raise NotImplementedError(
                        "where requires exactly 3 arguments")
                cond_expr = args[0]
                x_expr, x_kind = args_with_kinds[1]
                y_expr, y_kind = args_with_kinds[2]
                if x_kind == 'tensor' or y_kind == 'tensor':
                    kind = 'tensor'
                elif x_kind == 'vector' or y_kind == 'vector':
                    kind = 'vector'
                else:
                    kind = 'scalar'
                if kind == 'tensor':
                    tmp = self._new_temp('WHT')
                    self._add_decl('DOUBLE COMPLEX', tmp, '(3,3)')
                    self._var_kinds[tmp] = 'tensor'
                    self._add_stmt(
                        f'{tmp} = MERGE({x_expr}, {y_expr}, {cond_expr})')
                    return tmp, 'tensor'
                elif kind == 'vector':
                    tmp = self._new_temp('WHV')
                    self._add_decl('DOUBLE COMPLEX', tmp, '(3)')
                    self._var_kinds[tmp] = 'vector'
                    self._add_stmt(
                        f'{tmp} = MERGE({x_expr}, {y_expr}, {cond_expr})')
                    ...
                    return (f'MERGE({x_expr}, {y_expr}, {cond_expr})',
                            'scalar')
            elif func_name == 'det':
    ...
    def _emit_zeros(self, node):
        """Translate ``tensor.zeros((3,3))`` / ``tensor.zeros((3,))``."""
        if len(node.args) != 1:
            raise NotImplementedError(
                "zeros requires exactly 1 shape argument")
        shape_node = node.args[0]
        if isinstance(shape_node, ast.Tuple):
            shape = tuple(int(elt.value) for elt in shape_node.elts)
        elif isinstance(shape_node, ast.Constant):
            shape = (int(shape_node.value),)
        else:
            raise NotImplementedError(
                "zeros shape must be a literal tuple or integer")
        if shape == (3, 3):
            tmp = self._new_temp('ZER')
            self._add_decl('DOUBLE COMPLEX', tmp, '(3,3)')
            self._var_kinds[tmp] = 'tensor'
            self._add_stmt(f'{tmp} = DCMPLX(0.0d0, 0.0d0)')
            return tmp, 'tensor'
        elif shape == (3,):
            tmp = self._new_temp('ZER')
            self._add_decl('DOUBLE COMPLEX', tmp, '(3)')
            ...
            raise NotImplementedError(
                f"zeros shape {shape} not supported; use (3,3) or (3,)")
...
def _generate_statev_read(sv_info):
    ...
    Follows Chester's pattern: if time(2)==0, use initial values;
    else read from STATEV array.
    ...
    lines.append('C     (Pattern: Chester umat_simpleFeFp.for)')
...
def _generate_stress_subroutine(material, mat_prefix, matrix_backend='iterative'):
    ...
    Argument ordering follows INTERNAL_VARIABLES_DESIGN.md:
...
def _generate_umat_wrapper(mat_prefix, nprops, sv_info=None):
    ...
    following Chester's umat_simpleFeFp.for pattern.
    """
    if sv_info is None:
        sv_info = OrderedDict()
    has_state = bool(sv_info)
    ...
    lines.append('C')
    lines.append(
        f'C     Generated by coupfe.codegen for material: {mat_prefix}')
...
def _generate_small_strain_umat_wrapper(
    ...
    lines.append(f'C     Generated by coupfe.codegen for material: {mat_prefix}')
    lines.append(f'C     NPROPS = {nprops}')
    if has_state:
        lines.append(f'C     NSTATV = {nstatv}')
    lines.append('C' + '=' * 70)
...
def generate_umat(material, output_path, mat_prefix=None,
    ...
    if not os.path.exists(tensor_ops_path):
        raise FileNotFoundError(f"shipped template missing: {tensor_ops_path}")
    ...
    hdr.append('C')
    hdr.append('C     Generated by coupfe.codegen v0.2.0')
    hdr.append(
