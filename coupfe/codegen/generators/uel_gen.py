...
def _generate_native_rk(weakform, mat_prefix, cfg):
    ...
    lines = []
    lines.append('C======================================================================')
    lines.append('C     coupfe_element_rk subroutine — generated by coupfe.codegen')
    lines.append(f'C     Material: {weakform._mat.__class__.__name__}')
    lines.append(f"C     Fields: {', '.join(field_names)}")
    lines.append(f'C     Element: {cfg.name.capitalize()}, NDOFEL = {ndofel}')
    lines.append('C======================================================================')
    lines.append('      SUBROUTINE coupfe_element_rk(COORDS, U, DU, PROPS,')
...
C     --- Gauss point field values ---
    ...
    lines.append('')
    lines.append('      RETURN')
    lines.append('      END SUBROUTINE coupfe_element_rk')
...
def _generate_native_rk_fbar(weakform, mat_prefix, cfg):
    ...
    lines.append(f"""
C======================================================================
C     coupfe_element_rk subroutine — generated by coupfe.codegen (F-bar {cfg.name.capitalize()})
C     Material: {weakform._mat.__class__.__name__}
C     Fields: {u_field}
C     Element: {cfg.name.capitalize()}, NDOFEL = {ndofel}, F-bar = ON
C======================================================================
...
def generate_element(weakform, output_path, element='Quad8', mat_prefix=None,
    ...
    if fbar:
        ...
        if len(_fnames) == 1 and isinstance(_flds[_fnames[0]], VectorField):
            ...
            if backend == 'native':
                wrapper_src = _generate_native_rk_fbar(weakform, mat_prefix, cfg)
            else:
        ...
        if backend == 'native':
            wrapper_src = _generate_native_rk(weakform, mat_prefix, cfg)
        else:
