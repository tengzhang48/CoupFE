...
_ARG_FORTRAN_TYPE = {
    ...
    'J_inel':  ('DOUBLE COMPLEX', 'J_inel',       'scalar'),
...
_RETURN_FORTRAN = {
    ...
    'inelastic_jacobian': ('DOUBLE COMPLEX', 'J_inel_out'),
...
def _generate_material_subroutine(material, method_name, mat_prefix,
    ...
    has_state = (bool(sv_info) and method_name == 'stress_PK1'
                 and any(p in state_old_names or p == 'dt' for p in params))
...
def _generate_cs_engine(weakform, mat_prefix):
    ...
    # tangent blocks (see phase0_threefield_v2.py). Identical results at
...
def _append_state_var_declarations(lines, sv_info, indent='      '):
    ...
    for name, entry in sv_info.items():
        lines.append(f'{indent}INTEGER, PARAMETER :: OFF_{name.upper()} = {entry["offset"]}')


def _append_svars_read(lines, sv_info, indent='      ', svars_name='SVARS'):
    """Read state variables for Gauss point kk from a state array."""
    if not sv_info:
        return

    lines.append(f'C       Read state variables from {svars_name}')
    ...
    for name, entry in sv_info.items():
        ...
        shape = entry['shape']
        if shape == 'scalar':
            lines.append(f'{indent}  {name}_old = {svars_name}(LOC + OFF_{name.upper()} + 1)')
        elif shape == 'vector':
            for i in range(3):
                lines.append(
                    f'{indent}  {name}_old({i+1}) = '
                    f'{svars_name}(LOC + OFF_{name.upper()} + {i + 1})')
        elif shape == 'tensor':
            for j in range(3):
                for i in range(3):
                    idx = off + j * 3 + i + 1
...
def _append_svars_write(lines, sv_info, indent='      ', svars_name='SVARS'):
    """Write updated state variables back to a state array using real parts only."""
    if not sv_info:
        return

    lines.append(f'C       Write updated state to {svars_name} (real part only)')
    for name, entry in sv_info.items():
        off = entry['offset']
        shape = entry['shape']
        if shape == 'scalar':
            lines.append(
                f'{indent}{svars_name}(LOC + OFF_{name.upper()} + 1) = DBLE({name}_new_z)')
        elif shape == 'vector':
            for i in range(3):
                lines.append(
                    f'{indent}{svars_name}(LOC + OFF_{name.upper()} + {i + 1}) = '
...
def _generate_uel(weakform, mat_prefix, cfg):
    ...
    lines = []
    lines.append('C======================================================================')
    lines.append('C     UEL subroutine — generated by coupfe.codegen')
...
def _emit_rhs_assembly(lines, weakform, fields, field_names, equations,
                       needs_grad, cfg, backend='abaqus_uel'):
    """Emit residual assembly code for all equations.

    The ``abaqus_uel`` backend assembles into ``RHS = -R`` (Abaqus sign
    convention); the ``native`` backend assembles the weak-form residual ``R``
    directly.
    """
    is_native = (backend == 'native')
    resid = 'R' if is_native else 'RHS'
    ridx = '' if is_native else ',1'
    t_sign = '+' if is_native else '-'
    s_sign = '+' if is_native else '-'
    f_sign = '-' if is_native else '+'
    ...
    for eq_name, eq_info in equations.items():
        ...
        if eq_meta['return_type'] == 'tensor':
            # Momentum: R(row)  += P_iJ * dN_a/dX_J * wdetJ (native)
            #           RHS(row,1) -= P_iJ * dN_a/dX_J * wdetJ (Abaqus)
            lines.append(f'        DO ii_v = 1, {nn}')
            lines.append(f'          DO i = 1, ndim')
            lines.append(f'            row = edof_{test_field}(i, ii_v)')
            lines.append(f'            DO j = 1, ndim')
            lines.append(f'              {resid}(row{ridx}) = {resid}(row{ridx})')
            lines.append(f'     &          {t_sign} P_real(i,j)*{dsh}(ii_v,j)*wdetJ')
            lines.append(f'            END DO')
            lines.append(f'          END DO')
            lines.append(f'        END DO')
            ...
            # Storage: native  += c_dot * N * wdetJ, Abaqus -= c_dot * N * wdetJ
            lines.append(f'        DO ii_v = 1, {nn}')
            lines.append(f'          row = edof_{test_field}(ii_v)')
            lines.append(f'          {resid}(row{ridx}) = {resid}(row{ridx})')
            lines.append(f'     &      {s_sign} {storage_name}*{sh}(ii_v)*wdetJ')
            # Flux: native -= j_R . dN/dX * wdetJ, Abaqus += j_R . dN/dX * wdetJ
            lines.append(f'          DO j = 1, ndim')
            lines.append(f'            {resid}(row{ridx}) = {resid}(row{ridx})')
            lines.append(f'     &        {f_sign} {flux_name}(j)*{dsh}(ii_v,j)*wdetJ')
            lines.append(f'          END DO')
            lines.append(f'        END DO')
...
def _generate_uel_fbar(weakform, mat_prefix, cfg):
    ...
    lines.append(f"""
C======================================================================
C     UEL subroutine — generated by coupfe.codegen (F-bar {cfg.name.capitalize()})
...
def _generate_cs_tangent_dPdF(weakform, mat_prefix):
    ...
    # Build call to stress sub
    call_args = ['Fz']
    for p in extra_params:
        if p in state_old_names:
            call_args.append(f'{p}_z')
        elif p == 'dt':
            call_args.append('dt')
        else:
...
# =====================================================================
# Native element wrapper generation
# =====================================================================

def _generate_native_rk(weakform, mat_prefix, cfg, entry='rk'):
    """Generate the native ``coupfe_element_rk`` subroutine.

    This mirrors ``_generate_uel`` but uses CoupFE's own ABI:
    ``SUBROUTINE coupfe_element_rk(coords, u, du, props, svars_in,
    R, K, svars_out, time, dtime)``.  The residual is the weak form
    (positive sign), and the tangent is ``K = dR/dU`` — identical to the
    Abaqus ``AMATRX`` because ``AMATRX = -dRHS/dU = dR/dU``.

    ``entry='r'`` emits ``coupfe_element_r`` instead: the residual-only
    twin (same R statements in the same order — bit-identical R and
    state update), with the K-only work removed: no tangent output
    arrays, no ``{mat_prefix}_cs_tangents`` engine call, and no
    ``_emit_amatrx_assembly``. The real material evaluation, field
    interpolation, SVARS read/update, and ``_emit_rhs_assembly`` are
    shared verbatim. The solver-level benefit depends on callback order and
    must be measured for the consuming application.
    """
    if entry not in ('rk', 'r'):
        raise ValueError("native entry must be 'rk' or 'r'")
    emit_k = (entry == 'rk')
    ndim = cfg.ndim
    fields = weakform.fields
    field_names = weakform.field_names
    ...
    state_old_names = {f'{name}_old' for name in sv_info}
    nstate_per_gp = _nstate_per_gp(sv_info) if sv_info else 0
    total_state = nstate_per_gp * cfg.n_gauss_points if sv_info else 0
    nsvars_total = max(total_state, 1)
    ...
    ename = 'coupfe_element_rk' if emit_k else 'coupfe_element_r'
    elabel = ('coupfe_element_rk' if emit_k
              else 'coupfe_element_r (residual-only twin)')

    lines = []
    lines.append('C======================================================================')
    lines.append(f'C     {elabel} subroutine — generated by coupfe.codegen')
    lines.append(f'C     Material: {weakform._mat.__class__.__name__}')
    lines.append(f"C     Fields: {', '.join(field_names)}")
    lines.append(f'C     Element: {cfg.name.capitalize()}, NDOFEL = {ndofel}')
    lines.append('C======================================================================')
    lines.append(f'      SUBROUTINE {ename}(COORDS, U, DU, PROPS,')
    if emit_k:
        lines.append('     &  SVARS_IN, R, K, SVARS_OUT, TIME, DTIME)')
    else:
        lines.append('     &  SVARS_IN, R, SVARS_OUT, TIME, DTIME)')
    lines.append('')
    lines.append('      IMPLICIT NONE')
    lines.append('')
    lines.append('C     --- CoupFE native element interface ---')
    lines.append(f'      INTEGER, PARAMETER :: ndim = {ndim}')
    lines.append(f'      INTEGER, PARAMETER :: NCORNER = {n_corner}')
    lines.append(f'      INTEGER, PARAMETER :: NNODE_E = {n_nodes}')
    lines.append(f'      INTEGER, PARAMETER :: NGP = {cfg.n_gauss_points}')
    lines.append(f'      INTEGER, PARAMETER :: NDOFEL = {ndofel}')
    lines.append(f'      INTEGER, PARAMETER :: NPROPS = {nprops}')
    lines.append(f'      INTEGER, PARAMETER :: NSVARS = {nsvars_total}')
    lines.append(f'      INTEGER, PARAMETER :: MCRD = {ndim}')
    if sv_info:
        lines.append(f'      INTEGER, PARAMETER :: NSTATE_PER_GP = {nstate_per_gp}')

    lines.append('')
    lines.append('      DOUBLE PRECISION, INTENT(IN) :: COORDS(MCRD,NNODE_E)')
    lines.append('      DOUBLE PRECISION, INTENT(IN) :: U(NDOFEL), DU(NDOFEL)')
    lines.append('      DOUBLE PRECISION, INTENT(IN) :: PROPS(NPROPS)')
    lines.append('      DOUBLE PRECISION, INTENT(IN) :: SVARS_IN(NSVARS)')
    lines.append('      DOUBLE PRECISION, INTENT(OUT) :: R(NDOFEL)')
    if emit_k:
        lines.append('      DOUBLE PRECISION, INTENT(OUT) :: K(NDOFEL,NDOFEL)')
    lines.append('      DOUBLE PRECISION, INTENT(OUT) :: SVARS_OUT(NSVARS)')
    lines.append('      DOUBLE PRECISION, INTENT(IN) :: TIME(2), DTIME')
    lines.append('')
...
C     --- Gauss point field values ---
    ...
    # Tangent output arrays (K-only; specs still computed for the rk twin)
    if emit_k:
        lines.append('')
        lines.append('C     --- Tangent arrays ---')
    tangent_specs = []
    ...
    declared_tangents = set()
    for spec in (tangent_specs if emit_k else []):
        tn = spec['tname']
        if tn in declared_tangents:
            continue
        declared_tangents.add(tn)
    ...
    # --- Extract coordinates ---
    lines.append(f'C     Extract {_d}D coordinates from COORDS(MCRD, NNODE_E)')
    lines.append('      DO ii_v = 1, NNODE_E')
    for idim in range(1, _d + 1):
        lines.append(f'        {cfg.coords_name}({idim}, ii_v) = COORDS({idim}, ii_v)')
    lines.append('      END DO')
    ...
    # --- Zero R (and K) and carry state forward ---
    if emit_k:
        lines.append('C     Zero R and K')
        lines.append('      DO i = 1, NDOFEL')
        lines.append('        R(i) = 0.0d0')
        lines.append('        DO j = 1, NDOFEL')
        lines.append('          K(i, j) = 0.0d0')
        lines.append('        END DO')
        lines.append('      END DO')
    else:
        lines.append('C     Zero R')
        lines.append('      DO i = 1, NDOFEL')
        lines.append('        R(i) = 0.0d0')
        lines.append('      END DO')
    lines.append('')
    lines.append('C     Carry state forward (updated per Gauss point if present)')
    lines.append('      DO i = 1, NSVARS')
    lines.append('        SVARS_OUT(i) = SVARS_IN(i)')
    lines.append('      END DO')
    lines.append('')

    # --- DTIME guard ---
    lines.append('C     DTIME guard (allow DTIME=0 for initial eval)')
    lines.append('      dt_safe = DTIME')
    lines.append('      IF (dt_safe .LT. 1.0d-14) dt_safe = 1.0d-14')
    lines.append('')
    ...
    for fname in field_names:
        ...
        if isinstance(f, VectorField):
            ...
            lines.append(f'{indent}  idx = idx + 1')
            lines.append(f'{indent}  {fname}_node(i, ii_v) = U(idx)')
            lines.append(f'{indent}  {fname}_old(i, ii_v) = U(idx) - DU(idx)')
            lines.append(f'{indent}  edof_{fname}(i, ii_v) = idx')
            lines.append(f'{indent}END DO')
        else:
            lines.append(f'{indent}idx = idx + 1')
            lines.append(f'{indent}{fname}_node(ii_v) = U(idx)')
            lines.append(f'{indent}{fname}_old(ii_v) = U(idx) - DU(idx)')
            lines.append(f'{indent}edof_{fname}(ii_v) = idx')
    ...
    _append_svars_read(lines, sv_info, indent='        ', svars_name='SVARS_IN')
    ...
    lines_real_eval = []
    lines_real_eval.append('')
    lines_real_eval.append('C       Real material evaluation (for R)')
    lines_real_eval.append('        CALL real2complex33(F, Fz_r)')
    lines_real_eval.append('        CALL real2complex33(F_old, Fold_r)')
    for sfn in scalar_fields_list:
        lines_real_eval.append(f'        {sfn}z_r = DCMPLX({sfn}_gp, 0.0d0)')
    ...
    lines.extend(lines_real_eval)
    lines.append('')
    _append_svars_write(lines, sv_info, indent='        ', svars_name='SVARS_OUT')
    if sv_info:
        lines.append('')

    # --- CS tangent engine call (K-only; skipped in the R entry) ---
    if emit_k:
        lines.append('C       CS tangent engine')
        cs_args = ['F']
        for sfn in sorted([fn for fn in field_names if isinstance(fields[fn], ScalarField)
    ...
    # --- R assembly ---
    lines.append('C       R assembly (weak-form signs)')
    _emit_rhs_assembly(lines, weakform, fields, field_names, equations,
                       needs_grad, cfg, backend='native')
    lines.append('')

    # --- K assembly (K-only; skipped in the R entry) ---
    if emit_k:
        lines.append('C       K assembly')
        _emit_amatrx_assembly(lines, weakform, fields, field_names, blocks,
                              tangent_specs, needs_grad, cfg, tangent_name='K')
        lines.append('')
    ...
    lines.append('')
    lines.append('      RETURN')
    lines.append(f'      END SUBROUTINE {ename}')

    src = '\n'.join(lines)
    # The shared tangent assembly emits into AMATRX; map it to the native
    # output array K.  Then rename loop variable 'k' so it does not clash
    # with output 'K' (Fortran is case-insensitive).
    src = src.replace('AMATRX', 'K')
    return re.sub(r'\bk\b', 'k1', src)


def _generate_native_rk_fbar(weakform, mat_prefix, cfg, entry='rk'):
    """Generate the native F-bar ``coupfe_element_rk`` subroutine.

    Mirrors ``_generate_uel_fbar`` but uses the native ABI and weak-form
    residual signs.  Tangent assembly is unchanged from the Abaqus F-bar
    implementation because ``K = dR/dU``.

    ``entry='r'`` emits ``coupfe_element_r`` instead: the residual-only
    twin (same R statements in the same order, so R is bit-identical),
    with every K-only computation removed — no ``Finv_all``/``dJbar_du``
    (Pass 1), no CS tangent/K assembly/``Q`` (Pass 2), no Pass 3 — and
    no ``K`` argument.  State handling (SVARS read/update) is identical,
    so the R entry can serve explicit residual-only evaluations. No universal
    performance claim is made; representative solver callback counts and
    ...
    arg_line2 = ('     &  SVARS_IN, R, K, SVARS_OUT, TIME, DTIME)' if emit_k
                 else '     &  SVARS_IN, R, SVARS_OUT, TIME, DTIME)')

    lines = []
    lines.append(f"""
C======================================================================
C     {elabel} subroutine — generated by coupfe.codegen (F-bar {cfg.name.capitalize()})
C     Material: {weakform._mat.__class__.__name__}
C     Fields: {u_field}
C     Element: {cfg.name.capitalize()}, NDOFEL = {ndofel}, F-bar = ON
C======================================================================
      SUBROUTINE {ename}(COORDS, U, DU, PROPS,
{arg_line2}
...
C     --- CoupFE native element interface ---
      ...
      INTEGER, PARAMETER :: NNODE_E = {n_nodes}
      INTEGER, PARAMETER :: NGP = {ngp}
      INTEGER, PARAMETER :: NDOFEL = {ndofel}
      INTEGER, PARAMETER :: NPROPS = {nprops}
      INTEGER, PARAMETER :: NSVARS = {nsvars_total}
      INTEGER, PARAMETER :: MCRD = {ndim}
    """)
    if sv_info:
        lines.append(f'      INTEGER, PARAMETER :: NSTATE_PER_GP = {nstate_per_gp}')
...
C     --- Complex temps for material evaluation ---
    ...
    """)
        ...
        # Residual-only declarations: everything the R statements touch,
        # nothing used solely by the tangent path.
        lines.append(f"""
...
C     --- Complex temps for material evaluation ---
    ...
    if emit_k:
        ...
        lines.append('      END DO')
    lines.append('')
    lines.append('C     Carry state forward')
    lines.append('      DO i = 1, NSVARS')
    lines.append('        SVARS_OUT(i) = SVARS_IN(i)')
    lines.append('      END DO')
    lines.append('')
    ...
    lines.append('          idx = idx + 1')
    lines.append('          u_node(i, ii_v) = U(idx)')
    lines.append('          u_old(i, ii_v) = U(idx) - DU(idx)')
    lines.append('          edof_u(i, ii_v) = idx')
    lines.append('        END DO')
    lines.append('      END DO')
    lines.append('')
    ...
    # --- Compute dJbar/du (K-only; skipped in the R entry) ---
    if emit_k:
        lines.append('C     Compute dJbar/du (element-level sum)')
        lines.append('      DO jj_v = 1, NNODE_E')
        lines.append('        DO k = 1, ndim')
    ...
    # --- Pass 2: Material, R, standard K, store Q ---
    lines.append('C     =============================================')
    if emit_k:
        lines.append('C     Pass 2: Fbar, material, R, K_std, Q')
    else:
        lines.append('C     Pass 2 (R-only): Fbar, material, R')
    lines.append('C     =============================================')
    lines.append('      DO kk = 1, NGP')
    lines.append('        alpha = (Jbar / J_all(kk))')
    lines.append('     &    ** (1.0d0 / DBLE(ndim))')
    ...
    # R assembly (positive weak-form sign)
    lines.append('C       R: r_a^i = P_bar_iJ * dN_a/dX_J * wdetJ')
    lines.append('        DO ii_v = 1, NNODE_E')
    lines.append('          DO i = 1, ndim')
    lines.append('            row = edof_u(i, ii_v)')
    lines.append('            DO j = 1, ndim')
    ...
    # CS tangent + K assembly + Q (K-only; skipped in the R entry)
    if emit_k:
        # CS tangent: A = dP/dFbar
        lines.append('C       CS tangent: A_iJkL = dP_bar/dFbar_kL')
        lines.append(f'        CALL {mat_prefix}_cs_tangent_dPdF(')
        cs_args = ['Fbar']
        for name in sv_info:
        ...
        # Standard K (scaled by alpha)
        lines.append('C       Standard K (alpha-scaled)')
        lines.append('        DO ii_v = 1, NNODE_E')
        lines.append('          DO jj_v = 1, NNODE_E')
        lines.append('            DO i = 1, ndim')
        lines.append('              row = edof_u(i, ii_v)')
    ...
    # --- Pass 3: F-bar correction (K-only; skipped in the R entry) ---
    if emit_k:
        lines.append('C     =============================================')
        lines.append('C     Pass 3: F-bar correction (rank-1 update)')
        lines.append('C     =============================================')
...
def generate_element(weakform, output_path, element='Quad8', mat_prefix=None,
                     fbar=None, element_config=None, formulation=None,
                     backend='native'):
    """
    Generate a complete element .for file from a WeakForm instance.
    ...
    Args:
        ...
        formulation: explicit formulation route: ``'standard'``,
                     ...
                     preserved.
        backend: ``'native'`` for CoupFE's own ABI or ``'abaqus_uel'``
                 for the Abaqus UEL export.
    """
    backend = (backend or 'native').lower()
    if backend not in {'native', 'abaqus_uel'}:
        raise ValueError(
            f"Unknown backend '{backend}'. Use 'native' or 'abaqus_uel'.")
    ...
    if fbar:
        ...
        if len(_fnames) == 1 and isinstance(_flds[_fnames[0]], VectorField):
            ...
            if backend == 'native':
                # rk entry + residual-only twin (coupfe_element_r): same R
                # statements, no K-only work.
                wrapper_src = (
                    _generate_native_rk_fbar(weakform, mat_prefix, cfg)
                    + '\n'
                    + _generate_native_rk_fbar(weakform, mat_prefix, cfg,
                                               entry='r'))
            else:
                wrapper_src = _generate_uel_fbar(weakform, mat_prefix, cfg)
        else:
            raise ValueError(
        ...
        if backend == 'native':
            # rk entry + residual-only twin (coupfe_element_r): same R
            # statements, no CS engine or K assembly.
            wrapper_src = (
                _generate_native_rk(weakform, mat_prefix, cfg)
                + '\n\n'
                + _generate_native_rk(weakform, mat_prefix, cfg, entry='r'))
        else:
            wrapper_src = _generate_uel(weakform, mat_prefix, cfg)
        fbar_label = ''

    element_name = cfg.name.capitalize()
    if backend == 'native':
        section_label = 'Section 1: coupfe_element_rk subroutine (native entry point)'
    else:
        section_label = 'Section 1: UEL subroutine (Abaqus entry point)'

    # Apply column-72 wrapping to generated code
    mat_src = '\n'.join(_wrap_lines(mat_src.split('\n')))
    cs_src = '\n'.join(_wrap_lines(cs_src.split('\n')))
    wrapper_src = '\n'.join(_wrap_lines(wrapper_src.split('\n')))

    # Initial-state guidance for Abaqus runs (UEL backend only)
    init_comment = ''
    if backend == 'abaqus_uel':
        schema = weakform._mat.state_schema
        if schema:
            lines = ['C     Initial state variable values (*INITIAL CONDITIONS, TYPE=SOLUTION)']
            per_gp = sum(entry['size'] for entry in schema.values())
            lines.append(f'C     NSTATE_PER_GP = {per_gp}')
            for name, entry in schema.items():
                init = entry['init']
                if entry['size'] == 1:
                    val = float(init)
                    lines.append(
                        f'C       {name}: offset {entry["offset"]}, init = {val}')
                else:
                    flat = np.asarray(init, dtype=float).ravel()
                    vals = ', '.join(f'{v}' for v in flat)
                    lines.append(
                        f'C       {name}: offset {entry["offset"]}, '
                        f'size {entry["size"]}, init = ({vals})')
            init_comment = '\n'.join(lines) + '\n'
...
def generate_uel(weakform, output_path, element='Quad8', mat_prefix=None,
                 fbar=None, element_config=None, formulation=None):
    """Generate a complete Abaqus UEL .for file (backend = ``abaqus_uel``)."""
    return generate_element(
        weakform, output_path, element=element, mat_prefix=mat_prefix,
        fbar=fbar, element_config=element_config, formulation=formulation,
        backend='abaqus_uel')
