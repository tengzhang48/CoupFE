    ...
    for sm, fm in _STORAGE_FLUX_PAIRS:
        ...
        if indefinite:
            _warnings.warn(
                ...
                f"flux.grad(eta); see skills/pitfalls.md.",
                OperatorSignWarning, stacklevel=3)
        elif verbose:
...
def _fd_tangent(material, method_name, field_var, state, eps=1e-7):
    ...
    if np.isscalar(val) or (isinstance(val, np.ndarray) and val.ndim == 0):
        ...
        val = np.asarray(val)
        in_shape = val.shape
        # Some real-valued spectral models intentionally return a complex
        # container even for real inputs. Preserve that dtype: assigning into
        # a float buffer would warn and, more importantly, silently discard a
        # genuinely nonzero imaginary finite-difference response.
        tangent = np.zeros(out_shape + in_shape, dtype=complex)
...
def run_verification(material, state=None, tol=1e-6, verbose=True):
    ...
    # 3. Major symmetry — only for stateless materials
    #    (plasticity algorithmic tangent is generally non-symmetric)
    if not material.has_state:
        ok = _check_major_symmetry(material, state, tol, verbose)
        all_pass = all_pass and ok
    elif verbose:
        print(f"  Major symmetry dP/dF: skipped "
              f"(non-symmetric for materials with state variables)")
