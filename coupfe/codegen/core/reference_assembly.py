...
import numpy as np
from coupfe.codegen.core.fields import VectorField, ScalarField
from coupfe.codegen.core.defs import FIELD_ARGS
from coupfe.codegen.core.weakform import EQUATION_INFO
...
def _state_args_for_gp(material, STATEV, kk):
    ...
    for name, size, init in _state_var_sizes(material):
        ...
        if size == 1:
            ...
            # Column-major (order='F') to match the Fortran kernel's STATEV
            # layout (umat_gen: idx = offset + j*3 + i).  C-order silently
            # transposes any *non-symmetric* tensor state (e.g. Fp).
        ...
        for (block_key, wrt_var), binfo in blocks.items():
            ...
            if row_asm == 'grad' and col_asm == 'grad':
                if isinstance(tf, VectorField) and wrt_kind == 'matrix':
                    ...
                    # Completeness guard: a grad×grad coupling whose test-field kind
                    # / wrt_kind combination has no branch would otherwise be SILENTLY
                    # skipped (→ a wrong K). Fail loudly instead. (The assembly is a
                    # hand-enumerated table over {value,grad}×{value,grad}×field-kind×
                    # wrt_kind; new weak-form term shapes can hit an unfilled cell.)
                    raise NotImplementedError(
                        "reference_assembly: unhandled grad×grad tangent block "
                        f"(test={type(tf).__name__}, wrt_kind={wrt_kind}) for "
                        f"({block_key}, {wrt_var}); add a branch or use B-matrix "
                        "assembly.")
                ...
                if wrt_kind == 'matrix':
                    # drp/dF or dcdot/dF: trial is the vector field u (F = grad u),
                    # tang (ndim, ndim) = d(value-resid)/dF[k, l]; col = u[k, b].
                    for a in range(nn_row):
                        for b in range(nn_col):
                            row = edof[test_field][a]
                            for k in range(ndim):
                    ...
                    # drp/dgrad_phi, dcdot/dgrad_c: trial is a SCALAR field whose
                    # GRADIENT appears (grad_s[l] = sum_b s[b] dsh[b, l]), so edof is
                    # 1-D and tang (ndim,) = d(value-resid)/d(grad_s[l]); col = s[b].
                    for a in range(nn_row):
                        for b in range(nn_col):
                            row = edof[test_field][a]
                            col = edof[trial_field][b]
                ...
                # Completeness guard for the outer {value,grad}×{value,grad} table:
                # an unrecognized (row_asm, col_asm) pair would otherwise be silently
                # dropped from K. Fail loudly so the next new weak-form term is caught.
                raise NotImplementedError(
                    "reference_assembly: unhandled tangent assembly pattern "
                    f"(row_asm={row_asm}, col_asm={col_asm}, wrt_kind={wrt_kind}) "
                    f"for ({block_key}, {wrt_var}); add a branch or use B-matrix "
                    "assembly.")
...
def _flatten_gp_state(material, new_state_by_gp, n_gp, STATEV):
    ...
    state_vars order, column-major F-flatten — matching `_state_args_for_gp`'s
    reshape and the Fortran kernel's STATEV layout (column-major tensors).  GPs
    ...
    for kk in range(n_gp):
        ...
        for name, size, init in sizes:
            if sd is not None and name in sd:
                val = np.real(np.asarray(sd[name], dtype=complex)).ravel(order='F')
            elif STATEV is not None:
                val = np.asarray(STATEV[off:off + size], dtype=float).ravel(order='F')
            else:
                val = np.asarray(defaults[f'{name}_old'], dtype=float).ravel(order='F')
