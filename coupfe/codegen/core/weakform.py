...
class WeakForm:
    ...
    # Names of declared fields that carry an `_old` history value
    # (reconstructed from U-DU, not stored in SVARS).  None means "all fields".
    field_history = None
    ...
    def _build_argument_metadata(self):
        ...
        # Which declared fields carry an _old history?  Default: all of them.
        cls_hist = self.__class__.field_history
        if cls_hist is None:
            history_fields = set(self._field_order) | set(self._local_field_order)
        else:
            history_fields = set(cls_hist)
            unknown = history_fields - set(self._field_order) - set(self._local_field_order)
            if unknown:
                raise ValueError(
                    f"{self.__class__.__name__}.field_history contains "
        ...
        for name in self._local_field_order:
            ...
            # Local/condensed fields are stored in SVARS, not reconstructed from
            # U-DU, so they do NOT get an _old history argument here.

        self._field_arg_defs = field_args
        self._history_args = history_args
        self._param_args = set(PARAM_ARGS)

        self._validate_state_vs_history()

    def _validate_state_vs_history(self):
        """
        Loud-refusal checks for the field_history vs state_vars split.

        - A state_vars key must not also be a field name (ambiguous storage).
        - A derivable history (field_history) must not be declared in state_vars.
        - A non-derivable state variable must not appear as a field_history name.
        """
        state_names = set(self._mat.state_schema.keys())
        # Nodal fields cannot share a name with a state variable.
        # LocalScalar fields (element-local, condensed, stored in SVARS by design)
        # are allowed to overlap with state_vars — that is their explicit contract.
        field_names = set(self._field_order)

        ambiguous = state_names & field_names
        if ambiguous:
            raise ValueError(
                f"{self.__class__.__name__}: state_vars keys cannot also be "
                f"nodal field names (ambiguous storage): {sorted(ambiguous)}"
            )

        history_bases = {name[:-4] for name in self._history_args
                         if name.endswith('_old')}
        # Local/condensed fields are stored state, not derivable history.
        history_bases -= set(self._local_field_order)
        derivable_in_state = state_names & history_bases
        if derivable_in_state:
            raise ValueError(
                f"{self.__class__.__name__}: these quantities are declared as "
                f"state_vars but are reconstructible from U-DU via field_history "
                f"(do not store derivable history in SVARS): "
                f"{sorted(derivable_in_state)}"
            )
