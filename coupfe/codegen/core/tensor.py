...
MATRIX_FUNCTIONS_DESIGN.md.  f2py material tests compare generated Fortran
against the Python oracle at representative states.

See MATRIX_FUNCTIONS_DESIGN.md for details.
...
def transpose(A):
    """Transpose of a matrix. Fortran: transpose33z."""
    return A.T


def sum(A):
    """Sum all elements of an array. Fortran: SUM."""
    return np.sum(A)


def where(condition, x, y):
    """Elementwise selection. Fortran: MERGE."""
    return np.where(condition, x, y)


def zeros(shape):
    """Array of zeros. Fortran: scalar DCMPLX(0.0d0,0.0d0) broadcast."""
