...
class Material:
    ...
    @classmethod
    def _normalize_state_vars(cls):
        """
        Build a declared state schema from ``state_vars``.

        Backward-compatible with the simple dict-of-init-values form:
            state_vars = dict(ep=0.0, Fp=np.eye(3))
        and supports the explicit form:
            state_vars = dict(ep={'init': 0.0, 'shape': ()},
                              Fp={'init': np.eye(3), 'shape': (3, 3)})

        Returns an OrderedDict: name -> {
            'init': value,
            'shape': tuple,
            'size': int (entries in the flat per-GP array),
            'quantity': 'scalar'|'vector'|'tensor',
            'offset': int (flat offset within the per-GP block),
        }
        """
        ...
        schema = OrderedDict()
        state_var_props = getattr(cls, 'state_var_props', {})
        prop_names = list(cls.props.keys())
        offset = 0
        for name, spec in raw.items():
            ...
            if shape == ():
                ...
                raise ValueError(
                    f"state_var '{name}' has unsupported shape {shape}. "
                    f"Supported: (), (3,), (3,3)."
                )
    ...
    def state_schema(self):
        """Declared state-variable layout schema (ordered dict)."""
        return self._normalize_state_vars()
    ...
    def refresh_metadata(self, field_args=None, history_args=None,
        ...
        self._validate_state_history_args()
    ...
    def _validate_state_history_args(self):
        """
        Warn if any method argument ``foo_old`` is neither a declared
        field_history nor a state_vars key.  Catches typos and drift.
        """
        state_bases = {name for name in self.state_vars}
        history_bases = {name[:-4] for name in self._history_args
                         if name.endswith('_old')}
        for method_name, info in self._methods.items():
            for pname in info['all_params']:
                if not pname.endswith('_old'):
                    continue
                base = pname[:-4]
                if base not in state_bases and base not in history_bases:
                    warnings.warn(
                        f"{self.__class__.__name__}.{method_name}() argument "
                        f"'{pname}' is neither a declared field_history nor a "
                        f"state_vars key; it will be treated as a parameter.",
                        stacklevel=3,
                    )
    ...
    def _make_state(self, **kwargs):
        ...
        for name, info in self._field_args.items():
            ...
            if kind == 'matrix':
                ...
                # Neutral default: no inelastic volume change.
                state[name] = 1.0
            else:
                state[name] = 0.0
    ...
    def verify(self, state=None, tol=1e-6, verbose=True):
        ...
        try:
            ...
            from coupfe.codegen.core.verify import run_verification
